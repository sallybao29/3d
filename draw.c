...
void generate_sphere( struct matrix * points, 
  ...
  double cir, rev, x, y, z;
  for (rev = 0; rev < 1.0001; rev += step){
    for (cir = 0; cir < 1.0001; cir += step){
      x = r * cos(M_PI * cir) + cx;
      y = r * sin(M_PI * cir) * cos(2 * M_PI * rev) + cy;
      z = r * sin(M_PI * cir) * sin(2 * M_PI * rev);
...
void generate_torus( struct matrix * points, 
  ...
  for (rev = 0; rev < 1.0001; rev += step){
    for (cir = 0; cir < 1.0001; cir += step){
      x = r1 * cos(M_PI * cir) + cx;
      y = r1 * sin(M_PI * cir + r2) * cos(2 * M_PI * rev) + cy;
      z = r1 * sin(M_PI * cir + r2) * sin(2 * M_PI * rev);
      add_point(points, x, y, z);
    }
  } 
}
...
void add_box( struct matrix * points,
  ...
  add_edge(points, x + width, y, z - depth, x + width, y - depth, z - depth);
